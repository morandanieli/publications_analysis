...
class GNews:
    def __init__(self, language="en", country="US", max_results=100, period=None, start_date=None, end_date=None,
                 exclude_websites=None, proxy=None, user_agent=USER_AGENT):
        """
        (optional parameters)
        ...
        :param user_agent: Custom 'User-Agent' header string
        """
    ...
    def _get_news(self, query):
        ...
        try:
            if self._proxy:
                ...
                feed_data = feedparser.parse(url, agent=self._user_agent, handlers=[proxy_handler])
            else:
                feed_data = feedparser.parse(url, agent=self._user_agent)
