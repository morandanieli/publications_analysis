...
class _Scholarly:
    ...
    def fill(self, object: dict, sections=[], sortby: str = "citedby",
             publication_limit: int = 0) -> Author or Publication:
    ...
    def _construct_url(self, baseurl: str, patents: bool = True,
        ...
        country_lang = '&ceid={}:{}&hl={}-{}&gl={}'.format(country,
                                                           language,
                                                           language,
                                                           country,
                                                           country) if (country and language) else ''
        if sort_by == "date":
        ...
        # improve str below
        return url + yr_lo + yr_hi + citations + patents + sortby + start + country_lang
    ...
    def get_journals(self, category='English', subcategory=None, include_comments: bool = False) -> Dict[int, Journal]:
        try:
            ...
            try:
                ...
                raise ValueError(
                    "Invalid subcategory: %s for %s. Choose one from %s" % (subcategory, category, cat.keys()))
        except KeyError:
        ...
        try:
            with open(filename, 'w') as f:
                ...
                for rank, journal in journals.items():
                    row = [journal['name'], journal['h5_index'], journal['h5_median']] + [
